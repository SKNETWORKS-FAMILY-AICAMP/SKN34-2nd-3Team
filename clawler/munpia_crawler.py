...
class MunpiaAsyncCrawler:
    """CollectionService 호환용 단건 비동기 크롤러 어댑터."""

    def __init__(
        self,
        db_manager: ERDCSVManager | None = None,
        *,
        ...
        project_root = Path(__file__).resolve().parents[1]
        resolved_data_dir = Path(data_dir) if data_dir else project_root / "db" / "data"
        resolved_audit_dir = Path(audit_dir) if audit_dir else project_root / "db" / "audit"
        manager = db_manager or ERDCSVManager(
            data_dir=resolved_data_dir,
            audit_dir=resolved_audit_dir,
            target_ids=[],
        )
        self._crawler = MunpiaCrawler(
            manager=manager,
            target_ids=[],
            cookies=cookies,
        )
        self.headers = self._crawler.default_headers
        self.stop_event = self._crawler.stop_event
        self.active_states = self._crawler.active_states
