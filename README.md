...
지도·비지도학습으로 안정적인 후보 분류 기준을 확보하지 못해 작품별 지표를 독립적으로 분석했다. 조회수 규모 점수, FREE 조회수 유지·기울기 점수, PAID 유지 점수, 댓글 독자 반응 점수를 0~100 범위로 맞춘 뒤 값이 존재하는 항목을 동일 가중 산술평균하고, 작품별 통합 평균 점수로 순위와 유료 전환 검토 후보를 분류했다.
