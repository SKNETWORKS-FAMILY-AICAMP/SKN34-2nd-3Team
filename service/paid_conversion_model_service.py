...
class PaidConversionModelResult:
    trained_count: int
    candidate_count: int
    model_mae: float


class PaidConversionModelService:
    def __init__(
        self,
        connection_factory: Callable[[], Any] | None = None,
        model_factory: Callable[[], Any] | None = None,
        validation_predictor: Callable[..., Any] = cross_val_predict,
    ) -> None:
        self._connection_factory = connection_factory or self.connect
        self._model_factory = model_factory or self.build_model
        self._validation_predictor = validation_predictor
    ...
    def train_and_predict_all(self) -> PaidConversionModelResult:
        connection = self._connection_factory()
        try:
            training = self.fetch_frame(connection, self.feature_query(training=True))
            candidates = self.fetch_frame(connection, self.feature_query(training=False))
            if len(training) < 100:
            ...
            train_x = self.prepare_features(training)
            candidate_x = self.prepare_features(candidates)
            model = self._model_factory()
            folds = KFold(n_splits=5, shuffle=True, random_state=42)
            validation_prediction = self._validation_predictor(
                model, train_x, target, cv=folds
            )
            ...
            self.save_predictions(
                connection, candidates, prediction, model_mae, len(training)
            )
            return PaidConversionModelResult(
